...
const Button = (props) => {
   ...
   return <button type={props.type} className={`${props.className}`} {...rest}>{props.children}</button>
};
...
const ButtonWithStyles = styled(Button)`
    ...
    cursor: pointer;
    &:hover{
        background: ${(props) => props.color ? lighten(0.1, props.color) : lighten(0.1, Color.default)};
    }
`;
