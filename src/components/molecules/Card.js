    ...
    return(
        <div className={`card text-center ${props.className}`} onClick={props.onClick}>
            <div className="card-block">
                ...
                <SubTitle className={`card-subtitle mb-2 ${props.new ? "" : "text-muted"}`}
                      color={`${(props.new)? Colors.text.white : Colors.text.normal}`}
                      text={props.date}/>
            </div>
        </div>
...
const CardWithStyled = styled(Card)`
    background-image: ${props => (props.new)? "linear-gradient( 135deg, #CE9FFC 0%, #7367F0 100%)" : Colors.card.normal};
    width: 13rem;
    float: left;
    margin: 0 10px 10px 0;
    cursor: pointer;
    &:hover {
       background: ${props => (props.new)? "" : darken(0.1, Colors.card.normal)};
    }
    &:last-child {
      margin: 0;
    }
